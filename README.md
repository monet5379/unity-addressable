...
- **Demo** (optional) — boot preload, place-label enter/leave, load/spawn playground (not a shipping catalog)

Copy only **`Assets/AddressableLayout`** into a game. `Assets/Demo` is reference-only.

**Milestones A–C (in repo):** Runtime is under `project/unity-addressable/Assets/AddressableLayout` (`ResourcesManager` + PathManager + Addressables). After opening that Unity project, run **Tools → Addressable Layout → Demo → Register Boot Sample** (boot + place samples + PathMeta), then Play. Refresh alone: **Tools → Addressable Layout → Refresh Paths**.
...
- **Filename is a global key** — PathMeta warns on duplicates (`Duplicate filename in PathMeta; keeping first path`); first path wins.
...
- **Sync `Load` (Play) = cache hit only** — miss returns null. Preload by label or use `LoadAsync`. Do not block the main thread with sync Addressables fetch on miss.
...
- **Resources and Addressables must not share the same filename** when dual scan is on — Resources wins.
...
Assets/AddressableLayout/
  Runtime/
  Editor/                         # Refresh Paths (B); Path Settings UI later (E)
